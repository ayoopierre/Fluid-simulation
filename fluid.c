...
void lin_solve(int b, float *x, float *updated_x, float a, float c, int iter, int width, int height){
...
void handle_diffusion(int b, int width, int height, float *x, float *updated_x, float diff, float dt){
    float a = dt * diff * (width - 2) * (height - 2);
    lin_solve(b, x, updated_x, a, 1 + 6 * a, 20, width, height);
}

void handle_projection(int width, int height, float *v_x, float *v_y, float *p, float *div){
    for(int i = 1; i < width - 1; i++){
        for(int j = 1; j < height - 1; j++){
            div[i + j * width] = -0.5 * (
                v_x[i + 1 + j * width] - 
    ...
    lin_solve(0, p, div, 1, 6, 20, width, height);

    for(int i = 1; i < width - 1; i++){
        for(int j = 1; j < height - 1; j++){
            v_x[i + j * width] -= 0.5 * (p[i + 1 + j * width] - p[i - 1 + j * width]) * width;
            v_y[i + j * width] -= 0.5 * (p[i + (j + 1) * width] - p[i + (j - 1) * width]) * height;
        }
    }
...
void set_bnd(int b, float *x, int width, int height){
    ...
    x[0] = (float)(x[1] + x[width]) / 2;
    x[width - 1] = (float)(x[width - 2] + x[2 * width - 1]) / 2;
    x[width * (height - 1)] = (float)(x[width - 2] + x[2 * width - 1]) / 2;
    x[width * height - 1] = (float)(x[width - 1 + (height - 1) * width] + x[width * height - 2]) / 2;
}

void update_fluid_state(Fluid *fluid){
    handle_diffusion(1, fluid->width, fluid->height, fluid->updated_density, fluid->v_x, fluid->viscosity, fluid->dt);
    handle_diffusion(2, fluid->width, fluid->height, fluid->updated_v_y, fluid->v_y, fluid->viscosity, fluid->dt);

    handle_projection(fluid->width, fluid->height, fluid->v_x, fluid->v_y, fluid->updated_v_x, fluid->updated_v_y);
    ...
    handle_projection(fluid->width, fluid->height, fluid->updated_v_x, fluid->updated_v_y, fluid->v_x, fluid->v_y);

    handle_diffusion(0, fluid->width, fluid->height, fluid->density, fluid->updated_density, fluid->diffusion_rate, fluid->dt);
    handle_advection(0, fluid->width, fluid->height, fluid->density, fluid->updated_density, fluid->v_x, fluid->v_y, fluid->dt);
}